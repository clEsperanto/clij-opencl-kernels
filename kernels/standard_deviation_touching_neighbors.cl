...
__kernel void standard_deviation_touching_neighbors (
...
) 
{
  ...
  for (x = 0; x < label_id; x++) {
    float value = READ_IMAGE(src_matrix, sampler, POS_src_matrix_INSTANCE(x, y, 0, 0)).x;
    if (value > 0) {
      value = READ_IMAGE(src_vector, sampler, POS_src_vector_INSTANCE(x + x_correction, 0, 0, 0)).x - average;
      sum = sum + value * value;
      count++;
    }
  }
  ...
  // assume the object is included in mean calculation
  float value1 = READ_IMAGE(src_vector, sampler, POS_src_vector_INSTANCE(label_id + x_correction, 0, 0, 0)).x - average;
  sum = sum + value1 * value1;
  count++;
  ...
  for (y = label_id + 1; y < label_count; y++) {
    float value = READ_IMAGE(src_matrix, sampler, POS_src_matrix_INSTANCE(x, y, 0, 0)).x;
    if (value > 0) {
      value = READ_IMAGE(src_vector, sampler, POS_src_vector_INSTANCE(y + x_correction, 0, 0, 0)).x - average;
      sum = sum + value * value;
      count++;
    }
  }
