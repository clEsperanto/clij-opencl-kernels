...
) 
{
  ...
  float sum = (float) READ_IMAGE(src, sampler, INITIAL_POS).x;
  for (int LOOP_COORD = 1; LOOP_COORD < AXIS_SIZE(src); ++LOOP_COORD) {
    sum += (float) READ_IMAGE(src, sampler, LOOP_POS).x;
  }
  
  float mean = sum / AXIS_SIZE(src);
  WRITE_IMAGE(dst, OUTPUT_POS, CONVERT_dst_PIXEL_TYPE(mean));
}
