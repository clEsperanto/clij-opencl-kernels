...
)
{
  ...
  for (int dx = -radius.x; dx <= radius.x; dx++) {
    ...
    for (int dy = -radius.y; dy <= radius.y; dy++) {
      ...
      for (int dz = -radius.z; dz <= radius.z; dz++) {
        ...
        float test = xSquared / squared.x + ySquared / squared.y + zSquared / squared.z;
  ...
  WRITE_IMAGE(dst, POS_dst_INSTANCE(x,y,z,0), CONVERT_dst_PIXEL_TYPE(maximumValue));
}
