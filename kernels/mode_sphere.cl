...
__kernel void mode_sphere(IMAGE_src_TYPE src, IMAGE_dst_TYPE dst,
                          const int scalar0, const int scalar1,
                          const int scalar2) {
  ...
  if (GET_IMAGE_WIDTH(src) > 1) {
    radius.x = (scalar0 - 1) / 2;
    squared.x = (float)(radius.x * radius.x);
  }
  if (GET_IMAGE_HEIGHT(src) > 1) {
    radius.y = (scalar1 - 1) / 2;
    squared.y = (float)(radius.y * radius.y);
  }
  if (GET_IMAGE_DEPTH(src) > 1) {
    radius.z = (scalar2 - 1) / 2;
    squared.z = (float)(radius.z * radius.z);
  }
  ...
  for (int dx = -radius.x; dx <= radius.x; ++dx) {
    ...
    for (int dy = -radius.y; dy <= radius.y; ++dy) {
      ...
      for (int dz = -radius.z; dz <= radius.z; ++dz) {
        ...
        if (xSquared / squared.x + ySquared / squared.y +
                zSquared / squared.z <=
            1.0) {
          const int x1 = coord.x + dx;
          const int x2 = coord.y + dy;
          const int x3 = coord.z + dz;
          if (x1 < 0 || x2 < 0 || x3 < 0 || x1 >= GET_IMAGE_WIDTH(src) ||
              x2 >= GET_IMAGE_HEIGHT(src) || x3 >= GET_IMAGE_DEPTH(src)) {
            continue;
          }
  ...
  WRITE_IMAGE(dst, POS_dst_INSTANCE(x, y, z, 0),
              CONVERT_dst_PIXEL_TYPE(max_pos));
}